...
$downloadURL = 'https://github.com/shouryan01/export-ms-teams-chats/archive/refs/heads/main.zip'
...
if ($IsMacOS -or $IsLinux) {
    pwsh -File "$extractedPath/$innerFolderName/Get-MicrosoftTeamsChat.ps1" -exportFolder $out -Verbose

} else {
    try {
        pwsh.exe -File "$extractedPath/$innerFolderName/Get-MicrosoftTeamsChat.ps1" -exportFolder $out -Verbose
    } catch [System.Management.Automation.CommandNotFoundException] {
        powershell.exe -File "$extractedPath/$innerFolderName/Get-MicrosoftTeamsChat.ps1" -exportFolder $out -Verbose
    }
}
